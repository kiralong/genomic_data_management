...
If you have multiple sequencing lanes and some did not work (for whatever reason) note here what went wrong and where the "failed" runs are. If you have a lot of sequencing data that is later not used in a final publication, make sure you make it clear which directory contains the raw data that was used in the final publication.
